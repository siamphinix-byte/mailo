...
<div class="px-4 sm:px-6 py-2 bg-white dark:bg-admin-sidebar dark:border-admin-border fixed z-20 w-[calc(100%-30px)] border-b">
    <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-2 min-w-0 text-sm">
            ...
            <span id="template-header-title" class="font-semibold text-gray-900 dark:text-white truncate">{{ !empty($name) ? $name : 'New Template' }}</span>
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
            ...
            @endif
            <a href="{{ route('customer.templates.index') }}" class="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Discard</a>
...
<div class="flex-1 min-h-0 overflow-hidden flex flex-col" x-data="templateImportModal(@js([
    ...
    <form id="unlayer-form" method="POST" action="{{ route('customer.templates.store') }}" class="hidden">
        @csrf
        <input type="text" name="name" id="name" required value="{{ old('name', $name ?? '') }}">
    ...
    @include('customer.templates.unlayer._custom_builder', ['requireInitialTemplateDetails' => true])
