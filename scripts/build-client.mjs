...
function shouldCopyFactory() {
    const ignored = new Set(['.git', '.cursor', 'node_modules', '.vite', 'client-build', 'packages']);
    ...
    return (src, stat) => {
        ...
        // Ignore installed addons directory
        if (rel.startsWith('storage' + path.sep + 'app' + path.sep + 'addons')) return false;
...
async function main() {
    ...
    if (zipEnabled) {
        ...
        const zipFileName =
            argValue('--zip-file') ?? `mailpurse_build_${version.replace(/[^a-zA-Z0-9._-]/g, '')}_${hash}.zip`;
        const zipPath = path.resolve(projectRoot, zipFileName);

        const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mailpurse-client-zip-'));
        ...
        const masterZipFileName = argValue('--master-zip-file') ?? 'Mailpurse-master.zip';
        const masterZipPath = path.resolve(projectRoot, masterZipFileName);

        await fs.rm(masterZipPath, { force: true });

        const masterTmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mailpurse-master-zip-'));
        ...
        const wpPluginDir = path.join(wpBaseDir, 'mailpurse-integration');

        if (!(await pathExists(wpPluginDir))) {
            throw new Error('Missing wordpress-plugin/mailpurse-integration directory');
        }

        const wpPluginZipPath = path.join(masterTmpDir, 'wp-mailpurse-integration.zip');
        run('zip', ['-r', wpPluginZipPath, 'mailpurse-integration'], { cwd: wpBaseDir });
        ...
        // Zip each addon from packages/ into the project root
        const packagesDir = path.join(projectRoot, 'packages');
        if (await pathExists(packagesDir)) {
            const addonDirs = await fs.readdir(packagesDir);
            for (const addonName of addonDirs) {
                const addonPath = path.join(packagesDir, addonName);
                const addonStat = await fs.lstat(addonPath);
                if (!addonStat.isDirectory()) continue;
                const addonZipPath = path.join(packagesDir, `${addonName}.zip`);
                await fs.rm(addonZipPath, { force: true });
                run('zip', ['-r', addonZipPath, addonName], { cwd: packagesDir });
                console.log(`Created ${addonName}.zip`);
            }
        }
